...
print("Training flat single-task (coarse) convnet on " + train_dataset_name)
print("")
...
inputs, outputs \
    = create_flat_singletask_coarse_model(n_input_hops, n_filters, n_hidden_units, kernel_size, pool_size, base_wd=base_wd)
...
model_name = "classify-flat-singletask-coarse-convnet"
...
with open(history_path, 'w') as csv_file:
    ...
    header = [
        "Epoch", "Local time",
        "Training loss", "Training coarse accuracy (%)",
        "Validation loss", "Validation coarse accuracy (%)"]
...
validation_data = localmodule.get_validation_data(
        ...
        tfr_str=tfr_str, structured=False, label_inputs=False, single_output="coarse")
...
for init_id in range(n_inits):
    ...
    training_streamer = localmodule.multiplex_tfr(
        ...
        partial_labels=False, structured=False, single_output="coarse",
