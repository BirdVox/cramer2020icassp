...
import localmodule
from models import create_flat_singletask_fine_model
...
print("Training flat single-task convnet on " + train_dataset_name)
print("")
...
inputs, outputs \
    = create_flat_singletask_fine_model(n_input_hops, n_filters, n_hidden_units, kernel_size, pool_size, base_wd=base_wd)
...
model_name = "classify-flat-singletask-convnet"
...
with open(history_path, 'w') as csv_file:
    ...
    header = [
        "Epoch", "Local time",
        "Training loss", "Training fine accuracy (%)",
        "Validation loss", "Validation fine accuracy (%)"]
...
validation_data = localmodule.get_validation_data(
        ...
        tfr_str=tfr_str, structured=False, label_inputs=False, single_output="fine")
...
for init_id in range(n_inits):
    ...
    training_streamer = localmodule.multiplex_tfr(
        ...
        partial_labels=False, structured=False,
        active_streamers=active_streamers, streamer_rate=streamer_rate,
        num_cpus=num_cpus, single_output="fine", align_perturb=align_perturb)
    ...
    val_acc = 100 * list(history_df["Validation fine accuracy (%)"])[-1]
    if val_acc > 20.0:
        break
