...
import localmodule
from models import create_hierarchical_composition_multiclass_model
...
parser.add_argument('--no-partial-labels', action='store_false',
                    dest='partial_labels')
...
parser.add_argument('--loss-weight-medium', type=float, default=1.0)
parser.add_argument('--loss-weight-coarse', type=float, default=1.0)
...
epochs = args.epochs
tfr_str = args.tfr
loss_weight_medium = args.loss_weight_medium
loss_weight_coarse = args.loss_weight_coarse
...
print("Training hierarchical composition (coarse-to-fine) multi-class convnet on " + train_dataset_name)
print("")
...
inputs, outputs \
    = create_hierarchical_composition_multiclass_model(n_input_hops, n_filters, n_hidden_units, kernel_size, pool_size, base_wd=base_wd)
...
model_name = "classify-hierarchical-composition-coarsetofine-multiclass-convnet"
...
with open(history_path, 'w') as csv_file:
    ...
    header = [
        "Epoch", "Local time",
        "Training loss",
        "Training coarse accuracy (%)",
        "Training medium accuracy (%)",
        "Training fine accuracy (%)",
        "Validation loss",
        "Validation coarse accuracy (%)",
        "Validation medium accuracy (%)",
        "Validation fine accuracy (%)",
    ]
...
def write_row(history_path, epoch, logs):
    with open(history_path, 'a') as csv_file:
        ...
        row = [
            ...
            "{:.3f}".format(100*logs.get('y_coarse_binary_accuracy')).rjust(7),
            "{:.3f}".format(100*logs.get('y_medium_categorical_accuracy')).rjust(7),
            "{:.3f}".format(100*logs.get('y_fine_categorical_accuracy')).rjust(7),
            "{:.16f}".format(logs.get('val_loss')),
            "{:.3f}".format(100*logs.get('val_y_coarse_binary_accuracy')).rjust(7),
            "{:.3f}".format(100*logs.get('val_y_medium_categorical_accuracy')).rjust(7),
            "{:.3f}".format(100*logs.get('val_y_fine_categorical_accuracy')).rjust(7)
        ]
...
losses = {
    "y_coarse": 'binary_crossentropy',
    "y_medium": 'categorical_crossentropy',
    "y_fine": 'categorical_crossentropy',
}

metrics = {
    "y_coarse": [binary_accuracy],
...
validation_data = localmodule.get_validation_data(
        ...
        tfr_str=tfr_str, structured=True, label_inputs=False)
...
for init_id in range(n_inits):
    ...
    training_streamer = localmodule.multiplex_tfr(
        ...
        aug_kind_str=aug_kind_str, tfr_str=tfr_str, partial_labels=partial_labels,
        label_inputs=False, structured=True,
    ...
    val_acc = 100 * list(history_df["Validation coarse accuracy (%)"])[-1]
    if val_acc > 20.0:
        break
...
results = {
    'train': localmodule.get_results_output(model, training_streamer, 64, structured=True),
    'valid': localmodule.get_results_output(model, validation_data, 64, structured=True)
}
