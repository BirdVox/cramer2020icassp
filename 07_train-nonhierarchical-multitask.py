...
import localmodule
from models import create_nonhierarchical_multitask_model
...
print("Training flat multitask convnet on " + train_dataset_name)
print("")
...
inputs, outputs \
    = create_nonhierarchical_multitask_model(n_input_hops, n_filters, n_hidden_units, kernel_size, pool_size, base_wd=base_wd)
...
model_name = "classify-flat-multitask-convnet"
...
for init_id in range(n_inits):
    ...
    training_streamer = localmodule.multiplex_tfr(
        ...
        partial_labels=False, structured=True, label_inputs=False,
    ...
    val_acc = list(history_df["Validation fine accuracy (%)"])[-1]
    if val_acc > 20.0:
        break
...
results = {
    ...
    'valid': localmodule.get_results_output(model, validation_data, 1, structured=False)
}
